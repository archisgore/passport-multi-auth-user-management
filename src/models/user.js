...
function sqlConditionsWithComparator(conditions, comparator, indexOffset) {
  ...
  const serializedConditions = keys.map((key, index) => `${key} ${comparator} $${index + indexOffset}`)
...
const User = {
  ...
  findOne: async (conditions) => {
    ...
    const [ equalConditions, equalValues ] = sqlConditionsWithComparator(conditions.eq, '=', 1)
    const [ greaterThanConditions, greaterThanValues ] = sqlConditionsWithComparator(conditions.gt, '>', equalValues.length + 1)
    const [ lessThanConditions, lessThanValues ] = sqlConditionsWithComparator(conditions.lt, '<', equalValues.length + greaterThanValues.length + 1)
    const serializedConditions = [...equalConditions, ...greaterThanConditions, ...lessThanConditions]
